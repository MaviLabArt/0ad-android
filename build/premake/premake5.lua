...
newoption { category = "Pyrogenesis", trigger = "android", description = "Enable Android cross-compiling mode (experimental)" }
...
if _OPTIONS["android"] then
	...
	if string.find(machine, "aarch64") == 1 then
		arch = "aarch64"
	...
	elseif string.find(machine, "arm") == 1 then
		...
		print("WARNING: Cannot determine Android architecture from compiler triplet, defaulting to arm")
		arch = "arm"
	end
...
if os.istarget("bsd") then
	link_execinfo = true
elseif os.istarget("linux") and not _OPTIONS["android"] then
...
function project_set_build_flags()
	...
	-- Android uses EGL/OpenGL ES contexts. Build GLES code path by default there.
	if _OPTIONS["gles"] or _OPTIONS["android"] then
	...
	if _OPTIONS["without-atlas"] then
		defines { "CONFIG2_ATLAS=0" }
	end
		...
		if not _OPTIONS["minimal-flags"] then
			...
			if os.istarget("linux") or os.istarget("bsd") then
				...
				if next(sanitizers) == nil then
					if _OPTIONS["android"] then
						-- Keep explicit DT_NEEDED entries (e.g. SDL2) for runtime dlopen linking.
						linkoptions { "-Wl,-z,relro" }
					else
...
function setup_all_libs ()
	...
	source_dirs = {
		...
		if _OPTIONS["android"] then
			-- tinygettext uses SDL_iconv wrapper on Android.
			table.insert(extern_libs, "sdl")
		end
	...
	if _OPTIONS["without-atlas"] then
		project("engine")
		removefiles { source_root.."ps/AtlasGameLoopStub.cpp" }
	end
	...
	if os.istarget("linux") then
		if _OPTIONS["android"] then
			local android_sysdep = source_root .. "lib/sysdep/os/android"
			if os.isdir(android_sysdep) then
...
function setup_main_exe ()
	...
	local extra_files = { "main.cpp" }
	if _OPTIONS["without-atlas"] then
		table.insert(extra_files, "ps/AtlasGameLoopStub.cpp")
	end
	...
	elseif os.istarget("linux") or os.istarget("bsd") then
		...
		if _OPTIONS["android"] then
			-- Required only for 32-bit ARM toolchains.
			if arch == "arm" then
