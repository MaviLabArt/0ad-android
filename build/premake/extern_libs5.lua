...
extern_lib_defs = {
	boost = {
		compile_settings = function()
			...
			elseif _OPTIONS["android"] then
				-- Android toolchain uses host boost headers staged in .android-boost.
				externalincludedirs { rootdir .. "/.android-boost" }
			end
	...
	cpp_httplib = {
		...
		link_settings = function()
			if _OPTIONS["android"] then
				-- Android builds consume cpp-httplib as header-only.
				return
			end
			if not _OPTIONS["with-system-cpp-httplib"] then
	...
	enet = {
		...
		link_settings = function()
			...
			elseif _OPTIONS["android"] then
				-- Android builds use the vcpkg static archive.
				links { "enet" }
			else
	...
	icu = {
		...
		link_settings = function()
			if os.istarget("windows") then
				...
				if _OPTIONS["android"] then
					-- Android static ICU split data symbols into a separate archive.
					links { "icudata" }
				end
			end
		end,
	...
	libcurl = {
		...
		link_settings = function()
			if os.istarget("windows") then
				...
				if _OPTIONS["android"] then
					-- vcpkg curl for Android is static and needs explicit OpenSSL deps.
					links { "ssl", "crypto" }
				end
			end
			add_default_links({
	...
	libxml2 = {
		...
		link_settings = function()
			if os.istarget("windows") then
				...
				if _OPTIONS["android"] then
					-- Android static libxml2 pulls these optional decompressors.
					links { "bz2", "brotlidec", "brotlicommon" }
				end
			end
		end,
	},
	...
	sdl = {
		compile_settings = function()
			...
			elseif _OPTIONS["android"] then
				externalincludedirs { rootdir .. "/build/android/SDL2/include" }
