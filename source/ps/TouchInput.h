...
public:
	...
	bool IsEnabled();
	/**
	 * Returns whether a touch gesture is currently active.
	 */
	bool IsActive() const;
...
private:
	...
	int ResolveFingerSlot(std::int64_t fingerId, bool create);
	void ReleaseFingerSlot(std::int64_t fingerId);
	...
	std::int64_t m_FingerSlotId[MAX_FINGERS];
