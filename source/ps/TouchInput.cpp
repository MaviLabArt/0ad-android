...
bool CTouchInput::IsActive() const
{
	return m_State != STATE_INACTIVE;
}

int CTouchInput::ResolveFingerSlot(const std::int64_t fingerId, const bool create)
{
	for (size_t i = 0; i < MAX_FINGERS; ++i)
		if (m_FingerSlotId[i] == fingerId)
	...
	for (size_t i = 0; i < MAX_FINGERS; ++i)
	{
		if (m_FingerSlotId[i] == -1)
		{
			m_FingerSlotId[i] = fingerId;
			return static_cast<int>(i);
		}
	}
...
void CTouchInput::ReleaseFingerSlot(const std::int64_t fingerId)
{
	for (size_t i = 0; i < MAX_FINGERS; ++i)
	{
		if (m_FingerSlotId[i] == fingerId)
...
{
	if (id < 0 || id >= static_cast<int>(MAX_FINGERS))
		return;
...
void CTouchInput::OnFingerMotion(int id, int x, int y)
{
	...
	if (!g_Game || !g_Game->GetView() || !g_Game->GetView()->GetCamera())
		return;
...
InReaction CTouchInput::HandleEvent([[maybe_unused]] const SDL_Event_* ev)
{
	...
	case SDL_FINGERMOTION:
	{
		...
		debug_printf("finger %s tid=%" PRId64 " fid=%" PRId64 " x=%f y=%f dx=%f dy=%f p=%f\n",
			...
			const std::int64_t fingerId = static_cast<std::int64_t>(ev->ev.tfinger.fingerId);
			const int slot = ResolveFingerSlot(fingerId, ev->ev.type != SDL_FINGERUP);
			if (slot == -1)
				return IN_HANDLED;

			if (ev->ev.type == SDL_FINGERDOWN)
				OnFingerDown(slot, g_xres * ev->ev.tfinger.x, g_yres * ev->ev.tfinger.y);
			else if (ev->ev.type == SDL_FINGERUP)
			{
				OnFingerUp(slot, g_xres * ev->ev.tfinger.x, g_yres * ev->ev.tfinger.y);
				ReleaseFingerSlot(fingerId);
			}
			else if (ev->ev.type == SDL_FINGERMOTION)
				OnFingerMotion(slot, g_xres * ev->ev.tfinger.x, g_yres * ev->ev.tfinger.y);
			return IN_HANDLED;
		}
	}
