...
#if OS_ANDROID
static void ApplyAndroidSoundBootstrap()
{
	// Desktop defaults are intentionally conservative; on mobile/tablet speakers
	// they can be effectively inaudible. Only lift levels when still inherited
	// from default.cfg (i.e. user has not customized sound settings).
	struct SoundDefault
	{
		const char* key;
		const char* value;
	...
	constexpr SoundDefault androidDefaults[] = {
		{"sound.mastergain", "1.0"},
		{"sound.musicgain", "1.0"},
		{"sound.ambientgain", "1.0"},
		{"sound.actiongain", "1.0"},
		{"sound.uigain", "1.0"},
	};

	for (const SoundDefault& entry : androidDefaults)
		if (g_ConfigDB.GetValueNamespace(CFG_USER, entry.key) == CFG_DEFAULT)
			g_ConfigDB.SetValueString(CFG_USER, entry.key, entry.value);
}
#endif
