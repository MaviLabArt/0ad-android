...
Paths::Paths(const CmdLineArgs& args)
{
	...
	else // OS-specific path handling
	{
		...
		const OsPath appdata = m_root / "appdata";
...
{
#if OS_ANDROID
	// Android launcher code sets this to app-scoped external storage.
	const char* androidRoot = getenv("PYROGENESIS_ROOT");
	if (androidRoot && androidRoot[0] != '\0')
		return OsPath(androidRoot) / "";

	return OsPath("/sdcard/0ad"); // fallback for older launchers that don't set PYROGENESIS_ROOT
#else
