...
bool CConfigDB::WriteFile(EConfigNamespace ns, const VfsPath& path) const
{
	...
	// User/mod config sets can exceed 1 MiB on mobile targets.
	const size_t buffersize = 8*MiB;
	...
	auto append = [&](const char* fmt, auto&&... args) -> bool
	{
		if (len >= buffersize)
		{
			LOGERROR("CConfigDB::WriteFile(): buffer exhausted while writing \"%s\"", path.string8());
			return false;
		}

		const size_t remaining = buffersize - len;
		const int written = sprintf_s(pos, remaining, fmt, std::forward<decltype(args)>(args)...);
		if (written < 0 || static_cast<size_t>(written) >= remaining)
		{
			LOGERROR("CConfigDB::WriteFile(): formatting failed while writing \"%s\"", path.string8());
			return false;
		}
	...
	{
		size_t i;
		if (!append("%s = ", p.first.c_str()))
			return false;
		for (i = 0; i + 1 < p.second.size(); ++i)
		{
			if (!append("\"%s\", ", EscapeString(p.second[i]).c_str()))
				return false;
		}
		if (!p.second.empty())
		{
