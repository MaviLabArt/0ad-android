...
void CCameraController::Update(const float deltaRealTime)
{
	...
	int edgeDistance = m_ViewScrollMouseDetectDistance;
#if OS_ANDROID
	// Increase edge detection on Android to account for gesture/navigation insets.
	// Use a small percentage of the screen so the edge zone is reachable.
	int minDim = g_xres < g_yres ? g_xres : g_yres;
	int androidEdge = minDim / 48;
	if (androidEdge < 16)
		androidEdge = 16;
	if (edgeDistance < androidEdge)
		edgeDistance = androidEdge;
#endif

	bool mouseActive = g_mouse_active;
#if OS_ANDROID
	mouseActive = true;
#endif

	if (mouseActive && edgeDistance > 0)
	{
		if (g_mouse_x >= g_xres - edgeDistance && g_mouse_x < g_xres)
			moveRightward += m_ViewScrollSpeed * deltaRealTime;
		else if (g_mouse_x < edgeDistance && g_mouse_x >= 0)
			moveRightward -= m_ViewScrollSpeed * deltaRealTime;

		if (g_mouse_y >= g_yres - edgeDistance && g_mouse_y < g_yres)
			moveForward -= m_ViewScrollSpeed * deltaRealTime;
		else if (g_mouse_y < edgeDistance && g_mouse_y >= 0)
			moveForward += m_ViewScrollSpeed * deltaRealTime;
	}
