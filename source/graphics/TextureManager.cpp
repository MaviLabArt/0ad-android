...
public:
	 ...
	 */
		...
		PrepareCacheKey(texture, hash, version);
			...
			// If conversion isn't available (e.g. no NVTT on Android), fall back to
			// loading the source texture directly so UI/game textures still render.
			if (!m_TextureConverter.ConvertTexture(texture, sourcePath, looseCachePath, settings))
			{
				LoadTexture(texture, sourcePath);
