...
// FIXME Hack to get around exclusive access to the sound device
	...
	if (m_Device)
		{
			using AlcDeviceResumeSoft = void (*)(ALCdevice*);
			if (auto* resumeDevice = reinterpret_cast<AlcDeviceResumeSoft>(
					alcGetProcAddress(m_Device, "alcDeviceResumeSOFT")))
				resumeDevice(m_Device);
		}
	#endif
...
void CSoundManager::PauseMusic(bool pauseIt)
{
	...
	{
#if OS_ANDROID
		// On some Android devices, fade-in can leave the source muted (-inf dB).
		// Resume with direct gain to guarantee audible playback.
		m_CurrentTune->SetGain(m_MusicGain);
...
#if OS_ANDROID
				// Avoid zero-gain fade-in startup on Android to prevent silent playback.
				m_CurrentTune->SetGain(m_MusicGain);
#else
				m_CurrentTune->SetGain(0);
#endif
...
#if OS_ANDROID
				m_CurrentEnvirons->SetGain(m_AmbientGain);
#else
