...
void ApplyAndroidEntryPointFallbacks()
{
#if OS_ANDROID && !CONFIG2_GLES
#define ASSIGN_GL_FALLBACK(extFn, coreFn) \
	do { if (!(extFn) && (coreFn)) (extFn) = reinterpret_cast<decltype(extFn)>(coreFn); } while (false)

	// Desktop-style ARB/EXT entry points are often missing on Android drivers
	// even when the core function is available.
	ASSIGN_GL_FALLBACK(glGenBuffersARB, glGenBuffers);
	ASSIGN_GL_FALLBACK(glDeleteBuffersARB, glDeleteBuffers);
	ASSIGN_GL_FALLBACK(glBindBufferARB, glBindBuffer);
	ASSIGN_GL_FALLBACK(glBufferDataARB, glBufferData);
	ASSIGN_GL_FALLBACK(glBufferSubDataARB, glBufferSubData);
	ASSIGN_GL_FALLBACK(glMapBufferARB, glMapBuffer);
	ASSIGN_GL_FALLBACK(glUnmapBufferARB, glUnmapBuffer);
	ASSIGN_GL_FALLBACK(glCompressedTexImage2DARB, glCompressedTexImage2D);

	ASSIGN_GL_FALLBACK(glGenFramebuffersEXT, glGenFramebuffers);
	ASSIGN_GL_FALLBACK(glDeleteFramebuffersEXT, glDeleteFramebuffers);
	ASSIGN_GL_FALLBACK(glBindFramebufferEXT, glBindFramebuffer);
	ASSIGN_GL_FALLBACK(glFramebufferTexture2DEXT, glFramebufferTexture2D);
	ASSIGN_GL_FALLBACK(glCheckFramebufferStatusEXT, glCheckFramebufferStatus);
	ASSIGN_GL_FALLBACK(glBlitFramebufferEXT, glBlitFramebuffer);
	ASSIGN_GL_FALLBACK(glDrawRangeElementsEXT, glDrawRangeElements);

	ASSIGN_GL_FALLBACK(glGenQueriesARB, glGenQueries);
	ASSIGN_GL_FALLBACK(glDeleteQueriesARB, glDeleteQueries);
	ASSIGN_GL_FALLBACK(glGetQueryivARB, glGetQueryiv);
	ASSIGN_GL_FALLBACK(glGetQueryObjectivARB, glGetQueryObjectiv);

#undef ASSIGN_GL_FALLBACK
#endif
}
...
std::unique_ptr<IDevice> CDevice::Create(SDL_Window* window)
{
...
#else
#if !OS_MACOSX && !CONFIG2_GLES
		ogl_Init(SDL_GL_GetProcAddress, nullptr, 0);
#else
		ogl_Init(SDL_GL_GetProcAddress);
#endif
#endif
	...
	ApplyAndroidEntryPointFallbacks();
