	...
	const CDeviceCommandContext::UploadBufferFunction& uploadFunction)
{
	...
	// Some GLES drivers don't expose map/unmap buffer entry points through GLAD.
	// Use a slower but safe CPU staging fallback instead of crashing on nullptr.
	if (!glMapBufferARB || !glUnmapBufferARB)
	{
		std::vector<u8> uploadData;
		uploadData.resize(dataSize);
		uploadFunction(uploadData.data());
		glBufferSubDataARB(target, dataOffset, dataSize, uploadData.data());
		ogl_WarnIfError();
		return;
	}
...
void BindFramebufferCompat(const GLenum target, const GLuint framebuffer)
{
...
void BlitFramebufferCompat(
	const GLint srcX0, const GLint srcY0, const GLint srcX1, const GLint srcY1,
	...
	ENSURE(width > 0 && height > 0);
		if (texture->GetType() == CTexture::Type::TEXTURE_2D)
		{
			...
			if (texture->GetFormat() == Format::R8G8B8A8_UNORM ||
				...
				texture->GetFormat() == Format::L8_UNORM)
		{
...
void CDeviceCommandContext::ResetStates()
{
	...
	m_Framebuffer = m_Device->GetCurrentBackbuffer(
		...
		Renderer::Backend::AttachmentLoadOp::DONT_CARE,
		Renderer::Backend::AttachmentStoreOp::DONT_CARE)->As<CFramebuffer>();
	BindFramebufferCompat(GL_FRAMEBUFFER_EXT, m_Framebuffer->GetHandle());
	ogl_WarnIfError();
}
	...
	const SGraphicsPipelineStateDesc& pipelineStateDesc, const bool force)
{
	...
	{
#if !CONFIG2_GLES
		// Some Android GL stacks expose desktop-like extensions but omit glPolygonMode.
		// Skip this state update when the entry point is unavailable.
		if (glPolygonMode)
		{
			glPolygonMode(
				GL_FRONT_AND_BACK,
	...
	[[maybe_unused]] const Sampler::Filter filter)
{
	...
	BindFramebufferCompat(GL_READ_FRAMEBUFFER_EXT, sourceFramebuffer->GetHandle());
	BindFramebufferCompat(GL_DRAW_FRAMEBUFFER_EXT, destinationFramebuffer->GetHandle());
	// TODO: add more check for internal formats.
	BlitFramebufferCompat(
...
void CDeviceCommandContext::EndFramebufferPass()
{
	...
	{
		BindFramebufferCompat(GL_FRAMEBUFFER_EXT, framebuffer->GetHandle());
		ogl_WarnIfError();
	}
