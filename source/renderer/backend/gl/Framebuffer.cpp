...
void GenFramebuffersCompat(const GLsizei n, GLuint* framebuffers)
{
...
void DeleteFramebuffersCompat(const GLsizei n, const GLuint* framebuffers)
{
	if (glDeleteFramebuffersEXT)
		glDeleteFramebuffersEXT(n, framebuffers);
	else if (glDeleteFramebuffers)
...
void FramebufferTexture2DCompat(
	const GLenum target, const GLenum attachment,
	const GLenum textarget, const GLuint texture, const GLint level)
{
...
GLenum CheckFramebufferStatusCompat(const GLenum target)
{
	...
	SDepthStencilAttachment* depthStencilAttachment)
{
	...
	GenFramebuffersCompat(1, &framebuffer->m_Handle);
	if (!framebuffer->m_Handle)
	{
		LOGERROR("Failed to create CFramebuffer object");
		return nullptr;
	}
	BindFramebufferCompat(GL_FRAMEBUFFER_EXT, framebuffer->m_Handle);
...
#endif
		FramebufferTexture2DCompat(GL_FRAMEBUFFER_EXT, GL_COLOR_ATTACHMENT0,
	...
	if (depthStencilAttachment)
	{
		...
		const GLenum textureTarget = depthStencilAttachmentTexture->GetType() == CTexture::Type::TEXTURE_2D_MULTISAMPLE ?
			GL_TEXTURE_2D_MULTISAMPLE : GL_TEXTURE_2D;
#endif
		FramebufferTexture2DCompat(GL_FRAMEBUFFER_EXT, attachment,
	...
	const GLenum status = CheckFramebufferStatusCompat(GL_FRAMEBUFFER_EXT);
	if (status != GL_FRAMEBUFFER_COMPLETE_EXT)
	{
		LOGERROR("CFramebuffer object incomplete: 0x%04X", status);
		BindFramebufferCompat(GL_FRAMEBUFFER_EXT, 0);
		return nullptr;
	}
...
{
	if (m_Handle)
		DeleteFramebuffersCompat(1, &m_Handle);
}
